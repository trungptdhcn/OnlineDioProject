//
// DO NOT EDIT THIS FILE, IT HAS BEEN GENERATED USING AndroidAnnotations.
//


package com.example.OnlineDio.network.impl;

import android.app.Activity;
import android.content.Context;
import android.view.View;
import com.example.OnlineDio.network.ParseInServer_;

public final class HomeParseFeedInServiceImpl_
    extends HomeParseFeedInServiceImpl
{

    private Context context_;

    private HomeParseFeedInServiceImpl_(Context context) {
        context_ = context;
        init_();
    }

    public void afterSetContentView_() {
        if (!(context_ instanceof Activity)) {
            return ;
        }
    }

    /**
     * You should check that context is an activity before calling this method
     * 
     */
    public View findViewById(int id) {
        Activity activity_ = ((Activity) context_);
        return activity_.findViewById(id);
    }

    @SuppressWarnings("all")
    private void init_() {
        if (context_ instanceof Activity) {
            Activity activity = ((Activity) context_);
        }
        parseInServer = new ParseInServer_();
    }

    public static HomeParseFeedInServiceImpl_ getInstance_(Context context) {
        return new HomeParseFeedInServiceImpl_(context);
    }

    public void rebind(Context context) {
        context_ = context;
        init_();
    }

}
